<template>
	<view class="campingTour">
		<scroll-view  v-show="true"  scroll-y="true" class="scroll-Y" >
			<view class="card" v-for="(item,index) in scenicSpotCampingTour" :key="item.id" @tap="goToCampingContent(item)">
				<span class='campingCount'>露营游记{{item.id}}:</span>
				<br>
...
<script>
	...
	export default{
		...
		computed:{
			...mapState('changeScenicSpot',['currentScenicSpot','scenicSpotCampingTour'])
		},
		methods:{
			getToIndex(){
				uni.switchTab({
			...
			goToCampingContent(item){
				uni.navigateTo({
					url:`./campingTourContent?title=${item.title}&content=${item.content}&imageSrc=${item.imageSrc}`
				})
			}
		}
	}
